    ...
    def _create_evaluation_infos(
        ...
        if all_results is not None:
            ...
            for control_name in control_names:
                cache_hits_df_ = (
                    controls_to_evaluate_df.sort(["model_realization", control_name])
                    .join_asof(
                        cache_hits_df_.sort(["model_realization", control_name]),
                        on=control_name,
                        by="model_realization",  # pre-join by model realization
                        tolerance=EPS,  # Same as np.allclose with atol=EPS
                    )
                    .filter(pl.col("realization").is_not_null())
                )

                cache_hits_df_ = cache_hits_df_.drop(
                    [s for s in cache_hits_df_.columns if s.endswith("_right")]
                )

                if cache_hits_df_.is_empty():
                    break

            cache_hits_df = cache_hits_df_ if not cache_hits_df_.is_empty() else None
