...
def test_serializing_and_deserializing_ensemble_experiment(
    ...
    runmodel_from_serialized = EnsembleExperiment.model_validate_json(
        runmodel_json := runmodel.model_dump_json()
    )

    assert runmodel_from_serialized.model_dump_json() == runmodel_json
...
def test_serializing_and_deserializing_ensemble_smoother(
    ...
    runmodel_from_serialized = EnsembleSmoother.model_validate_json(
        runmodel_json := runmodel.model_dump_json()
    )
...
def test_serializing_and_deserializing_ensemble_information_filter(
    ...
    runmodel_from_serialized = EnsembleInformationFilter.model_validate_json(
        runmodel_json := runmodel.model_dump_json()
    )
...
def test_serializing_and_deserializing_esmda(
    ...
    runmodel_from_serialized = MultipleDataAssimilation.model_validate_json(
        runmodel_json := runmodel.model_dump_json()
    )
...
def _create_and_verify_runmodel_snapshot(config, snapshot, cli_args, case):
    ...
    # Override these to avoid user env-specifics in snapshots
...
cases_to_test = [
    ...
    "snake_oil_field/snake_oil_field.ert",
    # "snake_oil_field/snake_oil_surface.ert", # Missing irap file?..
]
...
def test_serializing_manual_update_snapshot(case, copy_case, snapshot):
...
@pytest.mark.parametrize("case", cases_to_test)
def test_serializing_evaluate_ensemble_snapshot(case, copy_case, snapshot):
    config_dir, config_file = case.split("/")
    copy_case(config_dir)
...
@pytest.mark.parametrize("case", cases_to_test)
def test_serializing_ensemble_experiment_snapshot(case, copy_case, snapshot):
    config_dir, config_file = case.split("/")
    copy_case(config_dir)
...
@pytest.mark.parametrize("case", cases_to_test)
def test_serializing_ensemble_smoother_snapshot(case, copy_case, snapshot):
    config_dir, config_file = case.split("/")
    copy_case(config_dir)
...
@pytest.mark.parametrize("case", cases_to_test)
def test_serializing_enif_snapshot(case, copy_case, snapshot):
    config_dir, config_file = case.split("/")
    copy_case(config_dir)
...
@pytest.mark.parametrize("case", cases_to_test)
def test_serializing_esmda_snapshot(case, copy_case, snapshot):
    config_dir, config_file = case.split("/")
    copy_case(config_dir)
